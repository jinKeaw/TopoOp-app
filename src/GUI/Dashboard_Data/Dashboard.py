...
from src.Environment.Env import load_p, sgen_p, gen_mw, env, num_lines, num_switches, network, profiles, time_step, limit
...
def into_step(action):
    env = Env.CustomEnv(num_lines, num_switches, network, profiles, time_step, limit)

    state, reward, done, truncated, info = env.step(action)
    # num_cong = env.num_congested
    # l_max = env.line_max
    # ll_max = "{:.2g}".format(env.lineload_max)
    return
    # return num_cong, l_max, ll_max
    ...
    dcc.Tabs(id="tabs-example-graph", value='tab-1', children=[
        dcc.Tab(label='Overview', value='tab-1', children=[
            dcc.Tabs(id="subtabs", value="subtab1", children=[
                dcc.Tab(label='Profiles', value='tab-1.1'),
                dcc.Tab(label='Grid View & Congestion', value='tab-1.2')
            ])
        ]),
        dcc.Tab(label='Environment', value='tab-2'),
        dcc.Tab(label='Agent', value='tab-3', children=[
            html.Div([dcc.Graph(id='graph-metric-cumu-reward', figure=fig_metric_cumulative_reward),
        ...
        if subtabs == 'tab-1.1':
            ...
            return (
                ...
                html.Div([
                    ...
                    html.Span(id='num-congested', children= num_cong),
                    html.Br(),
                    html.Span(children=['Line with the Highest Line Loading: ']),
                    html.Span(id='line-max', children= lin_max),
                    html.Br(),
                    html.Span(children=['Highest Line Loading(%): ']),
                    html.Span(id='lineload-max', children= ll_max),
                    html.Br(),
                    html.Button('Step', id='button_step', n_clicks= click),
...
def update_output(n_clicks):
    ...
    if n_clicks != 0:
        num_cong, lin_max, ll_max= update_step()
    ...
    return num_cong, lin_max, ll_max
