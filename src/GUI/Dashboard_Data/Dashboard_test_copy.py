...
table_data = [
    {"Substation": "", "Bus-1": "","Bus-2": "", "Line": "", "Switch-ID": "", "Switching Action": "","Type of Switch": "", "Measure Taken": ""}]
...
def handle_button_click(n_clicks, sw_table):
    ...
    if n_clicks and n_clicks > 0:
        ...
        GlobalState.action, GlobalState.num_congested_op, GlobalState.line_max_op, GlobalState.lineload_max_op, GlobalState.avg_ll_congested_op, sw_id,  sw_bus, sw_type, sw_substation = result
        # Simulate a loading process
        while len(sw_table) < len(sw_substation):
            sw_table.append({"Substation": "", "Bus-1": "", "Bus-2": "", "Line": "", "Switch-ID": "", "Switching Action": "", "Type of Switch": "", "Measure Taken": ""})

        while len(sw_table) > len(sw_substation):
            sw_table.pop()  # Removes the last row in sw_table
        # Update sw_table with values from sw_substation
        for i, (switch_id, switch_type, bus, substation) in enumerate(zip(sw_id, sw_type, sw_bus, sw_substation)):
