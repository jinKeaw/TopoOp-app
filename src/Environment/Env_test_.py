...
class CustomEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None, ts=None):
        ...
        initial_continuous = np.concatenate([
            self.gen_data_normalized.values[self.profiles_time_step].astype(np.float32),
            self.sgen_data_normalized.values[self.profiles_time_step].astype(np.float32),
            self.load_data_normalized.values[self.profiles_time_step].astype(np.float32)
        ], axis=0)
    ...
    def reward_calc(self, line_loadings_percent, limit):
        ...
        if ll_max >= 1:
            ...
            # Set any margins smaller than 1-limit to 0 (lines that have "margin < 1 - limit", have ll<1)
...
model =PPO.load(r"C:\Users\Jin\PycharmProjects\DRL_TopoOp\src\Environment\Agent.zip", env = env, deterministic = True)
...
def run_opt():
    ...
    sw_table_bf = env.network.switch
    #action_result.append({'Previous Action':previous_action.tolist() })
    #action_result.append({'Previous Action': previous_action})
    ...
    sw_table_af = env.network.switch
    action_table_op = action
    ...
    line_max_op = lineload_op[lineload_op == lineload_max_op].index[0]
    ...
    sw_all_closed = np.full(4500, True)
    sw_index_change = np.where(action_table_op != sw_all_closed)[0]
    print(sw_table_af.columns)
    sw_id = sw_table_af.iloc[sw_index_change]['name'].values
    sw_bus = sw_table_af.iloc[sw_index_change]['bus'].values
    sw_type = sw_table_af.iloc[sw_index_change]['type'].values
    sw_substation= sw_table_af.iloc[sw_index_change]['substation'].values
    sw_subnet = sw_table_af.iloc[sw_index_change]['subnet'].values
    sw_action = sw_table_af.iloc[sw_index_change]['closed'].values
    #print("sw_table",sw_table_af.iloc[sw_index_change])

    env.close()
    return action, num_congested_op, line_max_op , lineload_max_op,avg_ll_congested_op, sw_id,  sw_bus, sw_type, sw_substation,sw_subnet, sw_action
